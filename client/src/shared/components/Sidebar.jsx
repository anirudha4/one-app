...
function Sidebar() {
    ...
    return (
        <div className="sidebar bg-white">
            ...
            <div className="links flex flex-col mt-2">
                {NAVBAR_LINKS.map(NAV_LINK => {
                    return (
                        <NavLink key={NAV_LINK.id} to={NAV_LINK.route} className={({ isActive }) => `
                            p-3 border-r-2 border-transparent flex items-center gap-2
                            font-medium text-slate-600 transition duration-100
                            hover:bg-slate-100
                            ${isActive && 'border-slate-800 text-black bg-slate-50'}
                        `}>

 ...
 * Example usage:
 * ```
 *     return res.categoryNotFound();
 *     // -or-
 *     return res.categoryNotFound(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function categoryNotFound(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.categoryNotFound()');
    return res.json({ code: statusCodeToSet, message: 'Category not found' });
  }
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.categoryNotFound()` called with an Error:', optionalData);
    ...
    if (!_.isFunction(optionalData.toJSON)) {
      if (process.env.NODE_ENV === 'production') {
        return res.status(statusCodeToSet).json({ message: 'Category Not Found' })
      }
      else {
        return res.status(statusCodeToSet).json({ message: 'Category Not Found' })
      }

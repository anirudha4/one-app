/**
 * transactionNotFound.js
 *
 * A custom response.
 *
 * Example usage:
 * ```
 *     return res.transactionNotFound();
 *     // -or-
 *     return res.transactionNotFound(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function transactionNotFound(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.transactionNotFound()');
    return res.json({ code: statusCodeToSet, message: 'Transaction not found' });
  }
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.transactionNotFound()` called with an Error:', optionalData);
    ...
    if (!_.isFunction(optionalData.toJSON)) {
      if (process.env.NODE_ENV === 'production') {
        return res.status(statusCodeToSet).json({ message: 'Transaction Not Found' })
      }
      else {
        return res.status(statusCodeToSet).json({ message: 'Transaction Not Found' })
      }

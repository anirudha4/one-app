/**
 * memberAlreadyExists.js
 *
 * A custom response.
 *
 * Example usage:
 * ```
 *     return res.memberAlreadyExists();
 *     // -or-
 *     return res.memberAlreadyExists(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function memberAlreadyExists(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.memberAlreadyExists()');
    return res.json({ code: statusCodeToSet, message: 'Member already exists' });
  }
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.memberAlreadyExists()` called with an Error:', optionalData);
    ...
    if (!_.isFunction(optionalData.toJSON)) {
      if (process.env.NODE_ENV === 'production') {
        return res.status(statusCodeToSet).json({ message: 'Member already exists' })
      }
      else {
        return res.status(statusCodeToSet).json({ message: 'Member already exists' })
      }
